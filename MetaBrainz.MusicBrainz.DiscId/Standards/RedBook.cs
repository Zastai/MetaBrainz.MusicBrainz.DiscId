...
internal static class RedBook {
  ...
  public enum CDTextCharacterCode : byte {
    ...
    // Kanji (RIAJ RS506 - could not find the document; apparently Shift-Jis plus many emoji)
    MusicShiftJis = 0x80,

    // Encoding to be defined
    Korean = 0x81,

    // Encoding to be defined
    MandarinChinese = 0x82,
  ...
  public enum CDTextContentType : byte {
    ...
    // Album Title with ID2=0, Track Title otherwise
    Title = 0x80,
    ...
    // UPC/EAN with ID2=0, ISRC otherwise
    Code = 0x8e,
  ...
  public struct CDTextPack {
    ...
    // - If the same value is used for multiple tracks, the tab character (0x09, or 0x0909 for DBCS) can be used as string to
    //   indicate "same as previous".
    //   This is only applicable to content types 0x80-0x85, and should not be used for the first entry, nor for a track where the
    ...
    //   - 0x87     : Genre information, encoded as 2 bytes (MSB first) in the first two bytes of the text data. The supplementary
    //                description of the genre may be appended.
    ...
    //                One pack per interval, including a priority code. The priority code provides a sequence for the intervals
    //                (starting from 1).
    ...
    //                  MSF  Start;
    //                  MSF  End;
    //    - 0x8D    : Closed information. Strings for whole disc plus the tracks. Not to be shown nor read by players available to
    //                the public.
    //    - 0x8E    : UPC/EAN to be stored (typically as 13 characters) for track 0; ISRC to be stored (typically as 12 characters)
    //                for each track.
  ...
  public struct CDTextSizeInfo {
    ...
    // for blocks 0-7; 0 if there is no such block
    public byte[] LastSequenceNumber;

    [MarshalAs(UnmanagedType.ByValArray, SizeConst = 8)]
    // for blocks 0-7; language code is encoded as specified in annex 1 to part 5 of EBU Tech 3258-E.
    public EBU.LanguageCode[] LanguageCode;
