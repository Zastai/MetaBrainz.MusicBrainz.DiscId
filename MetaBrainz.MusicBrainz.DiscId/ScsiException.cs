...
public class ScsiException : ExternalException {
  ...
  internal ScsiException(SPC.FixedSenseData senseData)
    : this(senseData.SenseKey, senseData.AdditionalSenseCode, senseData.AdditionalSenseCodeQualifier) {
  }

  /// <summary>Creates a new <see cref="ScsiException"/> instance for the specified SCSI sense information.</summary>
  /// <param name="senseData">The SCSI sense data to derive the exception message from.</param>
  internal ScsiException(SPC.DescriptorSenseData senseData)
    : this(senseData.SenseKey, senseData.AdditionalSenseCode, senseData.AdditionalSenseCodeQualifier) {
  }

  /// <summary>
  /// The SCSI error code.
  /// The lower three bytes contain the sense key, additional sense code and additional sense code qualifier, respectively.
  /// </summary>
  public override int ErrorCode => (this.SenseKey << 16) | (this.AdditionalSenseCode << 8) | this.AdditionalSenseCodeQualifier;

  /// <summary>
  /// The SCSI error message.
  /// This is of the form &quot;A / B&quot;, where A is the description of the sense key and B is the description of the additional
  /// sense code.
  /// </summary>
  public override string Message {
    get {
      return string.Concat(SPC.SenseKeyDescription(this.SenseKey), " / ",
